...
The app reads subscription usage through the management API and displays Meta's five-hour and weekly remaining percentages and reset times. Unknown quota schemas display unavailable. If Meta omits the snapshot, VibeProxy retains the last reported values across restarts and labels them “Last known” with their timestamp. Without a saved snapshot, it says Meta has not reported usage yet. Refreshing never generates a model request. This is account-wide usage, including Muse CLI activity.
